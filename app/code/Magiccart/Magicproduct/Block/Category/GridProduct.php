...
class GridProduct extends \Magiccart\Magicproduct\Block\Product\ListProduct
{
    ...
    public function getBestrated($collection){
        ...
        $collection = $this->_productCollectionFactory->create();
        $collection->joinField(
                'rating_summary', 'review_entity_summary', 'rating_summary', 'entity_pk_value=entity_id', null, 'inner'
            );
        ...
        $collection->addAttributeToFilter('entity_id', ['in' => $producIds])
                ->groupByAttribute('entity_id')
                ->addAttributeToSort('rating_summary', 'desc')
                ->addStoreFilter()
                ->setPageSize($this->_limit)->setCurPage(1);            
